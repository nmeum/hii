...
func hasMonitor(client *girc.Client) bool {
	// XXX: Maximum amount of targets is currently not checked.
	_, success := client.GetServerOption("MONITOR")
	return success
}

func isMention(client *girc.Client, event *girc.Event) bool {
	// Don't check for mentions in NOTICE commands as they are
	// mostly automatically generated by bots (e.g. ChanServ).
...
func createListener(client *girc.Client, name string) (*ircDir, error) {
	...
	} else if girc.IsValidNick(name) && hasMonitor(client) {
	...
	client.Handlers.Add(girc.CONNECTED, func(c *girc.Client, e girc.Event) {
		for _, target := range flag.Args()[1:] {
			if girc.IsValidChannel(target) {
				...
				if hasMonitor(client) {
					c.Cmd.Monitor('+', target)
				} else {
					log.Printf("can't monitor %q, monitor not supported\n", target)
				}
			} else {
